...
\paragraph{Tiling.}
The raw raster is divided into non-overlapping training patches using a sliding-window tiler:
\begin{itemize}
...
\paragraph{Labels.}
Ground-truth annotations are sourced from the LROC feature catalogue (shapefiles and CSV), rasterised onto the tile grid as binary masks. To account for the point-like or line-like nature of some classes, morphological dilation is applied per class before rasterisation:
\begin{itemize}
...
Tiles are split 80/20 by sequential index, yielding 12{,}745 training tiles and 3{,}186 validation tiles.
...
\begin{enumerate}
    ...
    \item \textbf{CLAHE}: Contrast Limited Adaptive Histogram Equalization (clip limit $2.0$, grid size $8 \times 8$) to enhance local contrast and make faint features more visible.
...
This counter-intuitive result is explained by the geography of the training data. The Marius Hills region has a strong directional bias: wrinkle ridges run predominantly in a narrow range of orientations dictated by the regional stress field, and rilles follow the same orientation. Applying rotations and reflections to these tiles presents the model with orientations of these features that are physically absent in the training region — effectively injecting false diversity and confusing the direction-sensitive representations the model needs. The result is consistent with findings in remote sensing segmentation~\cite{chen2018augmentation} where domain-specific spatial priors can outweigh the benefits of generic augmentation. Impact craters, being rotationally symmetric, are unaffected by augmentation, which explains why crater AP is nearly identical in both conditions (0.947 vs 0.942).
...
\caption{Best-detected tile per class on the south pole imagery. Each panel shows the LRO WAC tile overlaid with the predicted probability map for the class with the highest maximum confidence in that tile. Impact craters stand out clearly; other classes show localised but low-confidence activations.}
...
\begin{enumerate}
    ...
    \item \textbf{Geometric augmentation is harmful on this dataset.} Training without flips and rotations outperforms the augmented model by 40\% on mean F1 and 31\% on mean AP. The Marius Hills training region has a directionally biased distribution of linear features; augmentation breaks this spatial prior and misleads the model. This result underlines that augmentation strategies must be tailored to the structural properties of the target domain.
