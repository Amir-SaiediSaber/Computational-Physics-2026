...
P/R/F1/IoU + MCC + ms/tile. YOLO is excluded (it collapsed to a single class and
cannot produce ridge-specific output).
...
COLORS = {"U-Net (semantic)": "#102A43", "Mask R-CNN (instance)": "#E87A2B", "Classical (Sato ridge)": "#9aa5b1"}
...
def main():
    ...
    order = ["U-Net (semantic)", "Mask R-CNN (instance)", "Classical (Sato ridge)"]
    ...
    for ax, metric, ttl in zip(axes, ["mcc", "f1", "iou"], ["MCC (skill)", "Pixel F1", "Pixel IoU"]):
        ...
        ax.set_xticks(range(len(order))); ax.set_xticklabels(["U-Net", "Mask R-CNN", "Classical"], fontsize=9)
        ax.set_title(ttl)
        if metric != "mcc":
    ...
    cols = ["Tile", "GT ridge", "U-Net", "Mask R-CNN", "Classical"]
