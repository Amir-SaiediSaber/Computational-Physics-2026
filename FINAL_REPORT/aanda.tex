...
Three controlled studies were run in sequence; Table~\ref{tab:final_summary} collects the best result of each.
...
\subsection{Audit, reproducibility, and definitive model}
\label{sec:mr_audit}

After the experimental campaign, the entire pipeline --- code, data, and report claims --- was audited systematically, with every change a single justified commit indexed in \texttt{IMPROVEMENTS.md}. The audit (a) reconciled the report against the code as ground truth (CLAHE parameters, overlapping tiling, split procedure, a gradient-clipping claim, a mis-attributed citation); (b) discovered the leakage problem --- 100\% of random-split validation tiles overlap at least one training tile --- and introduced the spatial block split of Sect.~\ref{sec:splits} together with the paired re-run above; (c) produced the label audit of Table~\ref{tab:prevalence}, the central reinterpretation of the section: without it the headline claim would have been the (nearly trivial) crater result, whereas \texttt{wrinkle\_ridge} is the strongest genuine one; and (d) fixed engineering defects while preserving published numbers, most notably making checkpoint loading loud on architecture mismatches --- previously a mismatch silently produced random predictions, a defect that resurfaces in Sect.~\ref{sec:southpole}.

The definitive model is SmallUNet with $w=32$, FocalDice, residual shortcuts, bottleneck dropout ($p=0.3$), trained \emph{without} augmentation for 30 epochs on the full dataset. One caveat: Studies 1--2 were run with augmentation enabled, before Study 3 established it is harmful; their selected components are architectural choices not expected to interact strongly with augmentation, but re-confirming those rankings in the no-augmentation regime remains the natural next experiment.
...
On the full spatial validation set the crater channel covers 82\% of pixels, and a \emph{predict-everything} baseline attains F1 $=0.90$ and IoU $=0.82$ with MCC $=0$. Table~\ref{tab:crater_allval} shows that the SmallUNet essentially reproduces this baseline (F1 $=0.90$, MCC $=0.01$): its high F1 is not evidence of skill --- the network has learned to output the base rate, which on a channel where half the tiles contain no negative pixels is very hard to beat. Any comparison ranking methods by F1 or IoU here would be ranking base-rate reproduction; this is why MCC is the protocol's primary metric.
